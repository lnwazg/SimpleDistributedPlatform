...
public class BETitlePane extends JComponent
{
    ...
    private void installSubcomponents()
    {
        ...
        //				||
        decorationStyle == JRootPane.INFORMATION_DIALOG || decorationStyle == JRootPane.ERROR_DIALOG || decorationStyle == JRootPane.COLOR_CHOOSER_DIALOG
            || decorationStyle == JRootPane.FILE_CHOOSER_DIALOG || decorationStyle == JRootPane.QUESTION_DIALOG || decorationStyle == JRootPane.WARNING_DIALOG)
        {
            createActions();
            createButtons();
            add(closeButton);
    ...
    private JMenu createMenu()
    {
        ...
        if (getWindowDecorationStyle() == JRootPane.FRAME || getWindowDecorationStyle() == JRootPane.PLAIN_DIALOG//现在也给dialog加上菜单项（但只有关闭项）
        )
        {
            addMenuItems(menu);
        }
    ...
    private void addMenuItems(JMenu menu)
    {
        ...
        if (getWindowDecorationStyle() == JRootPane.FRAME)//! 只有frame才有这些菜单项
        {
            ...
            if (Toolkit.getDefaultToolkit().isFrameStateSupported(Frame.MAXIMIZED_BOTH))
            {
                ...
                mnemonic = BEUtils.getInt("MetalTitlePane.maximizeMnemonic", -1);
                if (mnemonic != -1)
                {
                    mi.setMnemonic(mnemonic);
                }
    ...
    public static void paintTitlePane(Graphics g, int x, int y, int width, int height, boolean actived)
    {
        ...
        g2.setPaint(BEUtils.createTexturePaint(actived ? __IconFactory__.getInstance().getFrameTitleHeadBg_active().getImage() : __IconFactory__.getInstance()
            ...
            if (frame != null)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            List<Image> iis = (List<Image>)(ReflectHelper.invokeMethod(JFrame.class, frame, "getIconImages", new Class[] {}, new Object[] {}));
    ...
    private class TitlePaneLayout implements LayoutManager
    {
        ...
        public void layoutContainer(Container c)
        {
            ...
            if (menuBar != null)
            {
                ...
                menuBar.setBounds(x, y + 2//+2偏移量由Jack Jiang于2012-09-24日加上，目的是使得图标与文本保持在同一中心位置
                    ...
                    ,
                    IMAGE_HEIGHT,
                    IMAGE_WIDTH);//buttonWidth, buttonHeight);
            }
