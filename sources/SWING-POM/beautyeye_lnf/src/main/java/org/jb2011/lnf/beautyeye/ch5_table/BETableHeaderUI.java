...
public class BETableHeaderUI extends BasicTableHeaderUI
{
    ...
    */
    ...
    public static void paintHeadCell(Graphics g, Dimension headCellSize)
    {
        ...
        __Icon9Factory__.getInstance().getTableHeaderCellBg1().draw((Graphics2D)g, 0, 0, w, h);//表头背景
        __Icon9Factory__.getInstance().getTableHeaderCellSeparator1().draw((Graphics2D)g, w - 2, 0, 4, h - 1);//表头右边的分隔线,h-1是为了让分隔线往上移一个像素，好看一点
    }
        ...
        */
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column)
        {
            ...
            
            {
                ...
                SortOrder sortOrder =
                    (SortOrder)ReflectHelper.invokeMethod(DefaultTableCellHeaderRenderer.class, this, "getColumnSortOrder", new Class[] {JTable.class,
                        int.class}, new Object[] {table, column});
                if (sortOrder != null)
                {
                    switch (sortOrder)
                    {
    ...
    private static class IconBorder implements Border, UIResource
    {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int width, int height)
        {
            ...
            //			x + left + (width - left - right - icon.getIconWidth()) / 2, y + top);
                ...
                x + left + width - right - icon.getIconWidth() - 2,
                y);
        }
    }
}
