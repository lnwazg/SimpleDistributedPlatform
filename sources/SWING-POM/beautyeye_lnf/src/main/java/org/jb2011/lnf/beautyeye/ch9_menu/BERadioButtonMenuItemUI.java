    ...
    public static class RadioButtonMenuItemIcon implements Icon, UIResource, Serializable
    {
        ...
        public void paintIcon(Component c, Graphics g, int x, int y)
        {
            ...
            if (b.isSelected() == true)
            {
                ...
                g.drawImage(selectedImg, x + (usedForVista ? 5 : -4)//* 注意：当用于windows平台专用主类且处于Vista及更高版win时要做不一样的处理哦
                , y + (usedForVista ? -3 : -4), null);
            }
            else
            {
                g.drawImage(normalImg, x + (usedForVista ? 5 : -4)//* 注意：当用于windows平台专用主类且处于Vista及更高版win时要做不一样的处理哦
                , y + (usedForVista ? -3 : -4), null);
            }
        }
