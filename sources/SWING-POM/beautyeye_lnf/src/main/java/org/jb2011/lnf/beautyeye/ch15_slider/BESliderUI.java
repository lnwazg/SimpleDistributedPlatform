...
public class BESliderUI extends BasicSliderUI
{
    ...
    public void paintTrack(Graphics g)
    {
        ...
        if (slider.getOrientation() == JSlider.HORIZONTAL)
        {
            ...
            {
                //轨道背景
                __Icon9Factory__.getInstance().getSliderTrack().draw((Graphics2D)g, 0, 0, cw, THUMB_HEIGHT_HORIZONAL);
                //轨道（填充到当前刻度值处）
                __Icon9Factory__.getInstance().getSliderTrack_forground().draw((Graphics2D)g, 0, 0, thumbRect.x, THUMB_HEIGHT_HORIZONAL);
            }
            else
            {
                //轨道背景
                __Icon9Factory__.getInstance().getSliderTrack_disable().draw((Graphics2D)g, 0, 0, cw, THUMB_HEIGHT_HORIZONAL);
                //轨道（填充到当前刻度值处）
                __Icon9Factory__.getInstance().getSliderTrack_forground_disable().draw((Graphics2D)g, 0, 0, thumbRect.x, THUMB_HEIGHT_HORIZONAL);
            }
        ...
        else
        {
            ...
            {
                //轨道背景
                __Icon9Factory__.getInstance().getSliderTrack_VERITICAL().draw((Graphics2D)g, 0, 0, THUMB_WIDTH_VERTICAL, ch);
                //轨道（填充到当前刻度值处）
                // TODO BUG：当前有个bug，即在SwingSets2演示中，当thumb高度较小时，轨道半圆形被画出，这可能父类中thumbRect中计算有部碮同，有时间再研究吧，官方以后版本或许能解决哦
                __Icon9Factory__.getInstance().getSliderTrack_VERTICAL_forground().draw((Graphics2D)g, 0, thumbRect.y, THUMB_WIDTH_VERTICAL, ch - thumbRect.y);
            }
            else
            {
                //轨道背景
                __Icon9Factory__.getInstance().getSliderTrack_VERITICAL_disable().draw((Graphics2D)g, 0, 0, THUMB_WIDTH_VERTICAL, ch);
                //轨道（填充到当前刻度值处）
                __Icon9Factory__.getInstance()
                    .getSliderTrack_VERTICAL_forground_disable()
    ...
    public void paintFocus(Graphics g)
    {
        ...
        BEUtils.drawDashedRect(g, focusRect.x, focusRect.y, focusRect.width, focusRect.height);
    }
    ...
    public void paintThumb(Graphics g)
    {
        ...
        {
            if (slider.getOrientation() == JSlider.HORIZONTAL)
                g.drawImage(slider.isEnabled() ? __IconFactory__.getInstance().getSliderTick1_notrangle().getImage() : __IconFactory__.getInstance()
                    .getSliderTick1_notrangle_disable()
                    .getImage(), 0, 0, null);
            else
                g.drawImage(slider.isEnabled() ? __IconFactory__.getInstance().getSliderTick1_notrangle_vertical().getImage() : __IconFactory__.getInstance()
                    .getSliderTick1_notrangle_VERTICAL_disable()
                    .getImage(), 0, 0, null);
        }
        else if (slider.getOrientation() == JSlider.HORIZONTAL)
        {
            g.drawImage(slider.isEnabled() ? __IconFactory__.getInstance().getSliderTick1().getImage() : __IconFactory__.getInstance()
                .getSliderTick1_disable()
                .getImage(), 0, 0, null);
        }
        else
        { // vertical
            g.drawImage(slider.isEnabled() ? __IconFactory__.getInstance().getSliderTick1_vertical().getImage() : __IconFactory__.getInstance()
                .getSliderTick1_VERTICAL_disable()
                .getImage(), 0, 0, null);
        }
    ...
    protected boolean isPaintNoTrangleThumb()
    {
        ...
        return (!slider.getPaintTicks() && paintThumbArrowShape == null) || paintThumbArrowShape == Boolean.FALSE;
    }
