...
public class BEUtils
{
     ...
     */
    public static void componentsOpaque(java.awt.Component[] comps, boolean opaque)
    {
        if (comps == null)
            return;
    ...
    public static void drawDashedRect(Graphics g, int x, int y, int width, int height, int arcWidth, int arcHeight, int separator_solid, int separator_space)
    {
        ...
        Stroke sroke = new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[] {separator_solid, separator_space}, 0);//实线，空白
        ((Graphics2D)g).setStroke(sroke);
        
        g.drawRoundRect(x, y, width - 1, height - 1 //* 一个很诡异的问题：使用BasicStroke实现虚线绘制后，似乎绘制的距形
    ...
    {
        return new Color(getColorInt(basic.getRed() + r), getColorInt(basic.getGreen() + g), getColorInt(basic.getBlue() + b),
    ...
    {
        fillTextureRoundRec(g2, baseColor, x, y, w, h, arc, 35);
    }
