 ...
 */
public class BETextFieldUI extends BasicTextFieldUI implements BgSwitchable, org.jb2011.lnf.beautyeye.BeautyEyeLNFHelper.__UseParentPaintSurported//WindowsTextFieldUI
{
    ...
    {
        return getComponent() != null && (!(getComponent().getBorder() instanceof UIResource) || !(getComponent().getBackground() instanceof UIResource));
    }
    ...
    protected void paintBackground(Graphics g)
    {
        ...
        if (!isUseParentPaint())
        {
            ...
            BETextFieldUI.paintBg(g, 0, 0, editor.getWidth(), editor.getHeight(), editor.isEnabled(), bg);
        }
    ...
    public static void paintBg(Graphics g, int x, int y, int w, int h, boolean enabled, NinePatch bg)
    {
        if (enabled)
            ...
            __Icon9Factory__.getInstance().getTextFieldBgDisabled().draw((Graphics2D)g, x, y, w, h);
    }
