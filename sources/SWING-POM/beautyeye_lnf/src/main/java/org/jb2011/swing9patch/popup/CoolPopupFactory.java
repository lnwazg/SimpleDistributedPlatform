    ...
    protected class TranslucentPopup extends Popup
    {
        ...
        protected void reset(Component owner, Component contents, int ownerX, int ownerY)
        {
            ...
            if (c instanceof JWindow)
            {
                ...
                //内容组件半透明
                com.sun.awt.AWTUtilities.setWindowOpacity(component, isTooltip ? 1.0f : isComboBoxPopup ? 0.95f : 0.95f);//0.85f : 0.95f);//0.8f : 0.95f);
                //				WindowTranslucencyHelper.setOpacity(component,
                //						isTooltip ? 1.0f : isComboBoxPopup?0.95f : 0.95f);//0.85f : 0.95f);//0.8f : 0.95f);
                
                //图片填充背景的内容面板
                NinePatchBgPanel imageContentPane = new NinePatchBgPanel().setN9(isTooltip ? NPIconFactory.getInstance().getTooltipBg() : isComboBoxPopup ? //是下拉框列表则使一个跟JScrollPane一样的背景图，好看！
                ...
                if (contents instanceof JComponent)
                {
                    ...
                    ((JComponent)contents).setBorder(isTooltip ? BorderFactory.createEmptyBorder(107, 27, 12, 27)//6,8,12,12)
                        : isComboBoxPopup ? BorderFactory.createEmptyBorder(6, 4, 6, 4) : BorderFactory.createEmptyBorder(5, 3, 6, 3));//10,2,10,4));//5,5,20,5));
                }
